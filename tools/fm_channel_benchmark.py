...
METRIC DISCIPLINE (any "x VARA" claim MUST satisfy all of these):
  - Same layer:   compare client-DELIVERED goodput, with compression applied
                  symmetrically to both modems (never wire/PHY-peak vs delivered).
  - Same cell:    the VARA bar must be MEASURED in the exact (channel, SNR) cell
                  Iris ran in -- same bandpass, de-emphasis, and noise. A bar from
                  a different (or clean) channel is not a valid comparison.
  - Same traffic: identical payload corpus.
  - Whole session: goodput over the whole session, not a peak burst.
  - VARA measured, not quoted: the bar comes from a head-to-head VARA-FM run
                  recorded in a bar table, NOT a hardcoded number from a datasheet.
  This tool is FAIL-CLOSED: with no measured bar for the current cell it prints
  Iris's absolute delivered goodput and "NO BAR", never a ratio. The ~480 vs ~2838 bps VARA-FM bar is an
  OPEN question -- do not resurrect a quoted peak here.
"""
...
def cell_key(bandpass, deemph_us, noise):
    """Canonical (channel, SNR) cell identifier for VARA-bar lookup.

    The cell is defined by the simulated channel: bandpass, de-emphasis, and
    noise amplitude (the SNR proxy). A VARA bar is only valid for the cell it
    was measured in.
    """
    bp = "none" if not bandpass else str(bandpass)
    return f"bp={bp};deemph_us={deemph_us:g};noise={noise:g}"


def load_vara_bar(path, bandpass, deemph_us, noise):
    """Return the MEASURED VARA-FM delivered-goodput bar (bps) for this exact
    (channel, SNR) cell, or None if no such measurement exists.

    Table format (JSON):
      {"cells": {"<cell_key>": {"vara_bps": <float>, ...}, ...}}
    or the flat form {"<cell_key>": <float>, ...}. Missing file, missing key,
    or a non-positive/unparseable bar all yield None -> fail-closed (NO BAR).
    """
    ...
    try:
        ...
        return None
    cells = table.get("cells", table) if isinstance(table, dict) else None
    if not isinstance(cells, dict):
        return None
    entry = cells.get(cell_key(bandpass, deemph_us, noise))
    if entry is None:
        return None
    bar = entry.get("vara_bps") if isinstance(entry, dict) else entry
    try:
        bar = float(bar)
...
Examples:
  ...
  %(prog)s --no-bandpass --no-deemphasis      # Clean VB-Cable (baseline)
    ...
    parser.add_argument("--vara-bar-json", default=None,
                       help="Path to a JSON table of MEASURED VARA-FM delivered "
                            "goodput bars, keyed by (channel, SNR) cell. A ratio "
                            "vs VARA is emitted ONLY when this table has a bar for "
                            "the exact cell of this run; otherwise the tool is "
                            "fail-closed (prints NO BAR + Iris absolute goodput).")
    ...
    # Compare vs VARA FM -- FAIL-CLOSED.
    # A ratio is emitted ONLY when a MEASURED VARA-FM delivered-goodput bar
    # exists for THIS exact (channel, SNR) cell (--vara-bar-json). No quoted
    # datasheet peak: comparing a wire/PHY peak to a clean delivered goodput is
    # the ~8.4x metric error this tool must not reproduce.
    cell = cell_key(bandpass, deemph_us, args.noise)
    vara_bps = load_vara_bar(args.vara_bar_json, bandpass, deemph_us, args.noise)
    print()
    print(f"  Iris delivered goodput: {result['throughput_bps']:.0f} bps  (cell: {cell})")
    if vara_bps is None:
        print(f"  vs VARA FM:")
        print(f"    NO BAR -- VARA-FM unbenchmarked in this cell (measure VARA "
              f"head-to-head; see PRIOR_ART_AND_MEASUREMENT.md)")
    else:
        ratio = result['throughput_bps'] / vara_bps
        print(f"  vs VARA FM (measured {vara_bps:.0f} bps in this cell):")
        if ratio >= 1.0:
            print(f"    >>> {ratio:.2f}x VARA FM ({result['throughput_bps']:.0f} vs {vara_bps:.0f} bps) <<<")
        else:
            print(f"    {ratio:.2f}x VARA FM ({result['throughput_bps']:.0f} vs {vara_bps:.0f} bps)")
    print("=" * 70)
