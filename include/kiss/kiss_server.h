...
public:
    ...
    // Start listening on the given port, bound to bind_addr (default loopback).
    // bind_addr accepts a dotted-quad IPv4 literal or "any"/"0.0.0.0" for all
    // interfaces; a malformed address makes start() fail rather than binding
    // INADDR_ANY. See parse_bind_address() in config.h.
    bool start(int port = 8001, const std::string& bind_addr = "127.0.0.1");
    void stop();
