...
enum class Modulation : uint8_t {
    ...
    // 32-QAM cross constellation (5 bits/symbol). Appended (value 6, NOT in
    // constellation-order) so the native-PHY ordering checks (mod > QAM256)
    // keep rejecting it — 32QAM is an OFDM-only rung (VARA FM narrow top gear).
    // The OFDM path reaches it via bits_per_carrier==5 → bits_to_modulation(5).
    QAM32   = 6,   // 5 bits/symbol (cross-32)
};
