...
public:
    ...
    // Precook: size the persistent decode scratch (workspace / LLR / SPA-Q) once
    // at startup so no decode heap-allocates on the audio capture thread. Called
    // from Modem::precook() at the end of init(). Idempotent.
    static void precook();
