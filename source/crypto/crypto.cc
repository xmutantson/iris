...
void CipherSuite::derive_session_key(const char* local_call, const char* remote_call,
    ...
    if (!x25519_ready_ || (mlkem_done && !mlkem_ready_)) {
        crypto_wipe(session_key_, sizeof(session_key_));
        encryption_active_ = false;
        pq_active_ = false;
        kx_phase_ = KxPhase::IDLE;
    ...
    size_t call_a_len = strlen(call_a);
    size_t call_b_len = strlen(call_b);
    if (call_a_len > sizeof(salt) - salt_len ||
        call_b_len > sizeof(salt) - salt_len - call_a_len) {
    ...
    memcpy(salt + salt_len, call_a, call_a_len);
    salt_len += call_a_len;
    memcpy(salt + salt_len, call_b, call_b_len);
    salt_len += call_b_len;
    ...
    if (!encryption_active_)
        kx_phase_ = mlkem_done ? KxPhase::HYBRID_DONE : KxPhase::X25519_DONE;
}
...
void CipherSuite::compute_key_confirmation(uint8_t tag_out[8]) {
    if (kx_phase_ != KxPhase::X25519_DONE &&
        kx_phase_ != KxPhase::HYBRID_DONE &&
...
int CipherSuite::encrypt(const uint8_t* in, int in_len, uint8_t* out, int out_capacity,
    ...
    if (!encryption_active_ || in_len <= 0 ||
        tag_size < 1 || tag_size > AUTH_TAG_SIZE) return -1;
...
int CipherSuite::decrypt(const uint8_t* in, int in_len, uint8_t* out, int out_capacity,
                          uint64_t batch_counter, uint32_t direction, int tag_size) {
    if (!encryption_active_ || tag_size < 1 || tag_size > AUTH_TAG_SIZE ||
        in_len <= tag_size) return -1;
...
void CipherSuite::activate() {
    if (kx_phase_ != KxPhase::X25519_DONE &&
        kx_phase_ != KxPhase::HYBRID_DONE) return;
