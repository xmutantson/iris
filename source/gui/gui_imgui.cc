...
static uint64_t g_packet_log_generation = 0;
...
void IrisGui::update(const ModemDiag& diag, IrisConfig& config,
    ...
    {
        int tp = diag.ofdm_active ? diag.phy_bps :
            net_throughput(diag.speed_level,
    ...
    if (show_packet_log_) {
        ...
        bool auto_scroll = false;
        {
            ...
            static uint64_t prev_generation = 0;
            if (g_packet_log_generation != prev_generation) {
                auto_scroll = true;
                prev_generation = g_packet_log_generation;
            }
        }
        ...
        if (ImGui::BeginTabBar("SettingsTabs")) {
            ...
            if (ImGui::BeginTabItem("Station")) {
                ...
                const char* modes[] = {"A - 1200 baud (Standard FM)", "B - 9600 baud (9600 FM)",
                                       "C - 9600 baud (SDR/Direct)"};
                int mi = config.mode == "C" ? 2 : (config.mode == "B" ? 1 : 0);
                if (ImGui::Combo("Channel Mode", &mi, modes, 3)) {
                    const char* mode_ids[] = {"A", "B", "C"};
                    config.mode = mode_ids[mi];
                    ...
                    if (ImGui::BeginCombo("##RadioModel", preview)) {
                        ...
                        for (int j = 0; j < (int)models.size(); j++) {
                            if (radio_filter[0] != '\0') {
                                ...
                                for (auto& c : fl) c = (char)tolower((unsigned char)c);
