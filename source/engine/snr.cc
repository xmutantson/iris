...
float estimate_snr(const std::complex<float>* tx_symbols,
    ...
    if (signal_power < 1e-12f)
        return noise_power < 1e-12f ? 0.0f : -60.0f;
...
float AGC::process(float sample) {
    ...
    float sample_power = sample * sample;
    float rms_power = rms_est_ * rms_est_;
    float rate = (sample_power > rms_power) ? attack_ : decay_;
    rms_est_ = std::sqrt(rms_power + rate * (sample_power - rms_power));
