...
// ACE (Active Constellation Extension) projection.
// Given the original TX carrier value and the clipped value, return the
// ACE-projected value. Outer constellation edges are allowed to extend
// outward (further from decision boundary), reducing PAPR without
// increasing BER. Inner points and pilot carriers are restored exactly.
//
// For square QAM (BPSK, QPSK, 16QAM, 64QAM, 256QAM), the decision
// boundaries form a grid. Each I/Q axis is treated independently:
// - If original component is on the outer edge (max |value| for that axis),
//   the clipped component may extend further from zero but not toward it.
// - Otherwise, the component is restored to its original value.
//
// Ref: Krongold & Jones, "PAR Reduction in OFDM via Active Constellation
// Extension", IEEE Trans. Broadcasting, 2003.
inline std::complex<float> ace_project(std::complex<float> original,
                                        std::complex<float> clipped,
                                        int bpc) {
    if (bpc <= 0) return original;  // unused carrier

    // For BPSK (bpc=1): symbols are ±1 (real axis only).
    // Both are "outer" — allow extension outward.
    // For QPSK (bpc=2): symbols at (±1,±1)/√2. All are outer corners.
    // For M-QAM: grid spacing = 2, range ±(√M-1). Outer = |component| == √M-1.
    // After normalization, the max component magnitude for M-QAM is:
    //   (√M - 1) / √(2(M-1)/3)  where M = 2^(bpc/2) per axis
    // But we don't need the exact value — just check if |component| is
    // at the max grid value. The max I or Q for square QAM with bpc bits
    // is (levels-1) where levels = 2^(bpc/2) per axis, normalized.
    // Simpler: a component is "outer" if no other grid point exists
    // further from zero on that axis. For M-QAM with grid ±{1,3,...,2L-1}
    // (L levels per axis), max = 2L-1.

    int levels_per_axis;
    float grid_max;  // max unnormalized grid coordinate
    ...
    switch (bpc) {
        ...
        case 2:  // QPSK: ±1/√2
            levels_per_axis = 1;
            grid_max = 1.0f;
            norm = 1.0f / std::sqrt(2.0f);
            break;
        case 4:  // 16QAM: ±{1,3}/√10
            levels_per_axis = 2;
            grid_max = 3.0f;
            norm = 1.0f / std::sqrt(10.0f);
            break;
        case 6:  // 64QAM: ±{1,3,5,7}/√42
            levels_per_axis = 4;
            grid_max = 7.0f;
            norm = 1.0f / std::sqrt(42.0f);
            break;
        case 8:  // 256QAM: ±{1,3,...,15}/√170
            levels_per_axis = 8;
            grid_max = 15.0f;
            norm = 1.0f / std::sqrt(170.0f);
            break;
        default: // 1024QAM etc: restore original (don't extend)
            return original;
    }

    float outer_threshold = (grid_max - 0.5f) * norm;  // halfway between max and next-inner

    float orig_i = original.real();
    float orig_q = original.imag();
    float clip_i = clipped.real();
    float clip_q = clipped.imag();

    float out_i, out_q;

    // I axis: outer if |orig_i| >= outer_threshold
    if (std::abs(orig_i) >= outer_threshold) {
        // Outer: allow extension (same sign, at least as far from zero)
        if (orig_i > 0)
            out_i = std::max(clip_i, orig_i);  // can go more positive
        else
            out_i = std::min(clip_i, orig_i);  // can go more negative
    } else {
        out_i = orig_i;  // inner: restore
    }

    // Q axis
    if (bpc == 1) {
        out_q = 0.0f;  // BPSK: Q is always 0
    } else if (std::abs(orig_q) >= outer_threshold) {
        if (orig_q > 0)
            out_q = std::max(clip_q, orig_q);
        else
            out_q = std::min(clip_q, orig_q);
    } else {
        out_q = orig_q;
    }
...
Modulation bits_to_modulation(int bpc) {
    switch (bpc) {
        ...
        case 5: return Modulation::QAM32;
...
ToneMap make_uniform_tone_map(uint8_t preset_id, int n_data_carriers, int nfft) {
    ...
    // KEEP IN SYNC with kUniformPresets[] (ofdm_frame.cc) and OFDM_SPEED_LEVELS[]
    // (speed_level.cc). Index i = preset id = O-level + 1; index 0 = waterfill.
    static const Preset presets[] = {
        ...
        {5, LdpcRate::RATE_5_8},     //  7 = O6: 32QAM r5/8  (NEW: VARA FM narrow top gear)
        {6, LdpcRate::RATE_5_8},     //  8 = O7: 64QAM r5/8
        {6, LdpcRate::RATE_3_4},     //  9 = O8: 64QAM r3/4
        {8, LdpcRate::RATE_5_8},     // 10 = O9: 256QAM r5/8
        {8, LdpcRate::RATE_3_4},     // 11 = O10: 256QAM r3/4
        {8, LdpcRate::RATE_7_8},     // 12 = O11: 256QAM r7/8
        {10, LdpcRate::RATE_3_4},    // 13 = O12: 1024QAM r3/4
        {10, LdpcRate::RATE_7_8},    // 14 = O13: 1024QAM r7/8
    };
    const int n_presets = (int)(sizeof(presets) / sizeof(presets[0]));

    int idx = (preset_id >= 1 && preset_id < n_presets) ? preset_id : 2;  // default QPSK r1/2
    if (idx == 0) idx = 2;  // waterfill fallback = QPSK r1/2

    tm.tone_map_id = (uint8_t)idx;
    ...
    tm.n_codewords = ofdm_level_codeword_count(idx - 1);
    return tm;
}
...
{
    if (config_.nfft <= 0 ||
        config_.cp_samples < 0 || config_.cp_samples > config_.nfft ||
        config_.pilot_carrier_spacing <= 0 ||
        config_.n_used_carriers < 8 ||
        config_.n_used_carriers != (int)config_.used_carrier_bins.size() ||
        config_.n_pilot_carriers != (int)config_.pilot_carrier_bins.size() ||
        config_.n_data_carriers != (int)config_.data_carrier_bins.size() ||
        config_.n_used_carriers !=
            config_.n_pilot_carriers + config_.n_data_carriers) {
        throw std::invalid_argument("invalid OFDM configuration");
    }
    ...
    for (int i = 0; i < config_.n_used_carriers; i++) {
        ...
        if (bin < 0 || bin >= config_.nfft) {
            throw std::invalid_argument("invalid OFDM carrier map");
        }
        const std::vector<int>& expected =
            (i % config_.pilot_carrier_spacing == 0)
                ? config_.pilot_carrier_bins : config_.data_carrier_bins;
        int& idx = (i % config_.pilot_carrier_spacing == 0) ? pilot_idx : data_idx;
        if (idx >= (int)expected.size() || expected[idx++] != bin) {
            throw std::invalid_argument("invalid OFDM carrier map");
        }
    }
}
...
// ----------------------------------------------------------------------------
//  generate_pilot_symbol: constant-modulus ZC reference on every used carrier
//  (pilot rows, tail symbol, leading noise symbol).
//  The old all-ones comb (constant magnitude, zero phase everywhere) was a
//  time-domain impulse: PAPR = 10*log10(n_used) flat, 19-22 dB with the
//  de-emphasis shaping — the FM deviation limiter clipped the channel-
//  estimation symbols FIRST. Standard practice everywhere else: 802.11 LTF
//  (fixed low-PAPR sequence), LTE UL DMRS (Zadoff-Chu), DVB-T pilots
//  (PRBS-modulated).
//  The demodulator undoes the known ZC phase (conj multiply) before its
//  pilot-row CPE/H updates; the root rule is shared (ofdm_pilot_zc_root).
// ----------------------------------------------------------------------------
std::vector<std::complex<float>> OfdmModulator::generate_pilot_symbol() {
    ...
    // TX de-emphasis: same curve as the preamble, sync word, data and comb
    // pilots. The channel estimator's H[k] includes the gain; every symbol
    // type carries the same gain, so it cancels during equalization.
...
{
    if (!payload || len == 0) {
        IRIS_LOG("[OFDM-TX] empty payload rejected");
        return {};
    }

    auto geometry = checked_ofdm_frame_geometry(config_, tone_map);
    if (!geometry) {
        IRIS_LOG("[OFDM-TX] invalid frame geometry — rejected before allocation");
        return {};
    }
    const LdpcRate fec = geometry->fec();
    ...
    const auto checked_capacity = ofdm_payload_capacity_bytes(*geometry);
    if (!checked_capacity ||
        *checked_capacity > static_cast<std::uint64_t>(INT_MAX))
        return {};
    const int total_capacity = static_cast<int>(*checked_capacity);

    if (len > static_cast<size_t>(total_capacity)) {
        IRIS_LOG("[OFDM-TX] payload %zu bytes exceeds %d-block capacity %d — rejected",
                 len, n_codewords, total_capacity);
        return {};
    }
    const int k = LdpcCodec::block_size(fec);
    const int max_payload_per_block = k / 8 - 4 - 2;
    ...
    if (coded_bits.size() != expected) {
        IRIS_LOG("[OFDM-TX] encoder extent mismatch: %zu bits != %llu geometry bits",
                 coded_bits.size(), static_cast<unsigned long long>(expected));
        return {};
    }
    ...
    // Uses stride 173 (prime, coprime to any practical frame length).
    {
        ...
        if (N > FREQ_TIME_STRIDE &&
            std::gcd(N, FREQ_TIME_STRIDE) != 1) {
            IRIS_LOG("[OFDM-TX] coded length %d is not coprime to interleave stride %d",
                     N, FREQ_TIME_STRIDE);
            return {};
        } else if (N > FREQ_TIME_STRIDE) {
    ...
    int n_data_symbols = static_cast<int>(geometry->data_symbol_count());
    if (coded_bits.size() != geometry->data_symbol_count() *
            geometry->coded_bits_per_data_symbol()) {
        IRIS_LOG("[OFDM-TX] coded-bit extent disagrees with checked geometry");
        return {};
    }
        ...
        for (int s = 0; s < n_data_symbols; s++) {
            for (int k = 0; k < tone_map.n_data_carriers; k++) {
                if (!config_.dft_spread &&
                    tone_map.bits_per_carrier[k] == 0) continue;
                int used_idx = 0;
                int data_count = 0;
    ...
    // Known pilot-row reference phases (unit modulus). Pilot-row symbols are
    // fully known, so the ACE/ICF restore below pins EVERY used carrier of a
    // pilot row to ref*g — it must use the SAME sequence generate_pilot_symbol
    // emits or the restore would corrupt the rows it protects.
    std::vector<std::complex<float>> pilot_row_ref =
        generate_zc_sequence(ofdm_pilot_zc_root(n_used), n_used);
    ...
    // For ACE: store original used-carrier values and data symbol index
    // (-1 for pilot rows). Used in step 13b to project clipped carriers.
    std::vector<std::vector<std::complex<float>>> orig_used_carriers;
    std::vector<int> sym_data_idx;  // index into data_sym_carriers, or -1
    int data_sym_count = 0;
    int pilot_rows_inserted = 0;
    for (int s = 0; s < n_data_symbols; s++) {
        // E3: the block-pilot tier (pilot_symbol_spacing=24) was DELETED. Its
        // symbol was byte-identical to the dense pilot row (both were
        // generate_pilot_symbol()), and block spacing 24 is an exact multiple of
        // row spacing 8, so every block pilot landed on a pilot-row symbol and
        // emitted TWO consecutive identical pilots. Pure duplication; the dense
        // pilot row at that same symbol carries identical information. Removed on
        // BOTH ends in lockstep (ofdm_demod.cc) and dropped from every
        // frame-length gate formula so the RX buffers exactly what the TX emits.
        // pilot_symbol_spacing stays as vestigial negotiated metadata
        // (wire/fingerprint untouched).
            ...
            (data_sym_count % config_.pilot_row_spacing) == 0)
        {
            ...
            orig_used_carriers.push_back({});
            sym_data_idx.push_back(-1);
            pilot_rows_inserted++;
        }
        auto used = build_used_carrier_symbols(data_sym_carriers[s]);
        orig_used_carriers.push_back(used);  // store for ACE
        sym_data_idx.push_back(s);
    ...
    if (static_cast<std::uint64_t>(pilot_rows_inserted) != geometry->pilot_rows()) {
        IRIS_LOG("[OFDM-TX] pilot-row count disagrees with checked geometry");
        return {};
    }

    // ---- 11. Tail symbol ----
    // Bind the known boundary reference to C. A shorter trial cannot mistake
    // a later data symbol (or a dense pilot row) for its own frame boundary.
    const int tail_root = ofdm_tail_zc_root(n_used, n_codewords);
    if (tail_root == 0) {
        IRIS_LOG("[OFDM-TX] no distinct tail reference for C=%d", n_codewords);
        return {};
    }
    auto tail_body = generate_zc_training_symbol(config_, tail_root);
    std::vector<std::complex<float>> tail;
    tail.reserve(static_cast<size_t>(config_.cp_samples + config_.nfft));
    tail.insert(tail.end(), tail_body.end() - config_.cp_samples, tail_body.end());
    tail.insert(tail.end(), tail_body.begin(), tail_body.end());
    ...
    int total_ofdm_symbols = static_cast<int>(
        v2::FrameGeometry::kFixedNonDataSymbols + geometry->data_symbol_count() +
        geometry->pilot_rows());
    ...
    // Use the same pilot symbol structure (known ZC reference on all
    // carriers) — the RX Schmidl-Cox correlator ignores it because it's a
    // single symbol (SC needs two identical symbols), and its ZC root differs
    // from the preamble's so the ZC timing/FD-ZC correlators reject it too.
    // Same amplitude as data symbols.
    ...
    if (frame.size() != geometry->complete_sample_count()) {
        IRIS_LOG("[OFDM-TX] serialized sample extent %zu != checked geometry %llu",
                 frame.size(),
                 static_cast<unsigned long long>(geometry->complete_sample_count()));
        return {};
    }
    ...
    if (config_.skip_papr_clip) {
        // Clipper disabled via config. Warning: on OTA Fe-Pi hardware,
        // disabling the clipper has been shown to HURT throughput (fact
        // doc §7.3). Keep this flag OFF unless testing on a platform
        // without ADC saturation concerns.
        skip_clip = true;
        papr_target = 99.0f;
    } else if (config_.dft_spread && max_bpc >= 4) {
        ...
        // No DFT-spread: ACE extends outer constellation points outward after
        // clipping, so inner point EVM is the limiting factor. Target ~10-11 dB
        // to keep EVM tolerable for higher-order QAM. Final PAPR after pilot
        // restoration will be ~12-13 dB (similar to DFT-spread + pilot restore).
        if (max_bpc >= 8)      papr_target = 11.0f;  // 256QAM
        else if (max_bpc >= 6) papr_target = 10.5f;  // 64QAM
        else if (max_bpc >= 4) papr_target = 10.0f;  // 16QAM
        else                   papr_target = 9.5f;    // BPSK/QPSK
    }
    int n_clipped = 0;
    float papr_before_db = 0.0f, papr_after_db = 0.0f;
    bool use_ace = !config_.dft_spread;

    if (!skip_clip) {
        int n_data_syms = (int)all_data_time.size();
        int ace_extended = 0, ace_restored = 0;

        if (use_ace) {
            // ---- Non-DFT-spread: Iterative ICF + ACE ----
            // Each iteration: clip → OOB filter → pilot restore + ACE project.
            // Including restoration inside the loop prevents restored carriers
            // from permanently spiking PAPR.
            const int n_ace_iter = 4;
            papr_before_db = compute_papr_db(data_part);

            std::vector<bool> keep_bin(config_.nfft, false);
            ...
            int sym_len_clip = config_.symbol_samples();
            int n_symbols = (int)data_part.size() / sym_len_clip;

            for (int iter = 0; iter < n_ace_iter; iter++) {
                float mean_power = 0.0f;
                for (auto& s : data_part) mean_power += std::norm(s);
                mean_power /= (float)data_part.size();
                if (mean_power < 1e-20f) break;
                float threshold = std::sqrt(mean_power * std::pow(10.0f, papr_target / 10.0f));

                int iter_clipped = 0;
                for (auto& s : data_part) {
                    float mag = std::abs(s);
                    if (mag > threshold) {
                        s *= (threshold / mag);
                        iter_clipped++;
                ...
                n_clipped += iter_clipped;
                if (iter_clipped == 0) break;

                // Per-symbol OOB filter
                std::vector<std::complex<float>> fbuf(config_.nfft);
                for (int sym = 0; sym < n_symbols; sym++) {
                    int sym_start = sym * sym_len_clip;
                    int body_start = sym_start + config_.cp_samples;

                    std::copy(data_part.begin() + body_start,
                              data_part.begin() + body_start + config_.nfft,
                              fbuf.begin());
                    fft_complex(fbuf.data(), config_.nfft);

                    for (int k = 0; k < config_.nfft; k++)
                        if (!keep_bin[k]) fbuf[k] = {0.0f, 0.0f};

                    fbuf[0] = {fbuf[0].real(), 0.0f};
                    if (config_.nfft > 1)
                        fbuf[config_.nfft / 2] = {fbuf[config_.nfft / 2].real(), 0.0f};
                    for (int k = 1; k < config_.nfft / 2; k++) {
                        auto avg = (fbuf[k] + std::conj(fbuf[config_.nfft - k])) * 0.5f;
                        fbuf[k] = avg;
                        fbuf[config_.nfft - k] = std::conj(avg);
                    }

                    ifft_complex(fbuf.data(), config_.nfft);
                    std::copy(fbuf.begin(), fbuf.end(),
                              data_part.begin() + body_start);
                    std::copy(data_part.begin() + body_start + config_.nfft - config_.cp_samples,
                              data_part.begin() + body_start + config_.nfft,
                              data_part.begin() + sym_start);
                }

                // Pilot restore + ACE project
                for (int sym = 0; sym < n_data_syms && sym < n_symbols; sym++) {
                    int sym_start = sym * sym_len_clip;
                    int body_start = sym_start + config_.cp_samples;
                    if (body_start + config_.nfft > (int)data_part.size()) break;
                    ...
                    bool modified = false;
                    bool has_orig = (sym < (int)orig_used_carriers.size() &&
                                     !orig_used_carriers[sym].empty());
                    // Pilot ROW (sym_data_idx == -1): every used carrier is a
                    // known ZC reference — restore them all, with the ZC phase.
                    bool row_pilot = (sym < (int)sym_data_idx.size() &&
                                      sym_data_idx[sym] < 0);

                    for (int i = 0; i < n_used; i++) {
                        int bin = config_.used_carrier_bins[i];

                        if (is_pilot[i] || row_pilot) {
                            float g = fm_tx_deemph_gain(bin, config_.nfft, config_.sample_rate,
                                                   config_.fm_preemph_corner_hz,
                                                   config_.fm_preemph_gain_cap);
                            // Comb pilots in data symbols are +1*g (zero
                            // phase); pilot-row carriers carry the ZC phase.
                            std::complex<float> ref = row_pilot
                                ? pilot_row_ref[i] : std::complex<float>(1.0f, 0.0f);
                            std::complex<float> known = ref * (g * (float)config_.nfft);
                            if (std::abs(fbuf[bin] - known) > 0.01f * std::abs(known)) {
                                fbuf[bin] = known;
                                int mirror = config_.nfft - bin;
                                if (mirror > 0 && mirror < config_.nfft)
                                    fbuf[mirror] = std::conj(known);
                                modified = true;
                            }
                        } else if (has_orig) {
                            std::complex<float> orig = orig_used_carriers[sym][i]
                                                       * (float)config_.nfft;
                            std::complex<float> clipped = fbuf[bin];

                            int data_k = 0;
                            for (int j = 0; j < i; j++)
                                if (!is_pilot[j]) data_k++;
                            int bpc = (data_k < tone_map.n_data_carriers)
                                      ? tone_map.bits_per_carrier[data_k] : 0;

                            std::complex<float> projected = ace_project(orig, clipped, bpc);
                            if (projected != clipped) {
                                fbuf[bin] = projected;
                                int mirror = config_.nfft - bin;
                                if (mirror > 0 && mirror < config_.nfft)
                                    fbuf[mirror] = std::conj(projected);
                                modified = true;
                                if (iter == n_ace_iter - 1) {
                                    if (projected != orig) ace_extended++;
                                    else ace_restored++;
                                }
                            }
                        }
            ...
            papr_after_db = compute_papr_db(data_part);
            IRIS_LOG("[OFDM-TX] ACE+ICF: %d extended, %d restored, PAPR=%.1f->%.1f dB (target %.1f)",
                     ace_extended, ace_restored, papr_before_db, papr_after_db, papr_target);
        } else {
            // ---- DFT-spread: standard ICF then single pilot restore ----
            ...
            if (n_clipped > 0) {
                int sym_len_r = config_.symbol_samples();
                std::vector<std::complex<float>> fbuf(config_.nfft);
                for (int sym = 0; sym < n_data_syms; sym++) {
                    int sym_start = preamble_samples + sym * sym_len_r;
                    int body_start = sym_start + config_.cp_samples;
                    ...
                    bool modified = false;
                    // Pilot ROW: every used carrier is a known ZC reference
                    // (restore all, with the ZC phase); data symbols restore
                    // only the zero-phase comb pilots.
                    bool row_pilot = (sym < (int)sym_data_idx.size() &&
                                      sym_data_idx[sym] < 0);
                    for (int i = 0; i < n_used; i++) {
                        if (!is_pilot[i] && !row_pilot) continue;
                        int bin = config_.used_carrier_bins[i];
                        float g = fm_tx_deemph_gain(bin, config_.nfft, config_.sample_rate,
                                               config_.fm_preemph_corner_hz,
                                               config_.fm_preemph_gain_cap);
...
std::vector<std::complex<float>> OfdmModulator::build_ofdm_frame(
    const uint8_t* payload, size_t len, ToneMap& tone_map, LdpcRate fec) {
    if (fec != tone_map.fec_rate) {
        IRIS_LOG("[OFDM-TX] frame shape mismatch: arg fec=%d, map fec=%d",
                 static_cast<int>(fec), static_cast<int>(tone_map.fec_rate));
        return {};
    }
    tone_map.n_codewords = 1;
    return build_ofdm_frame(payload, len,
                            static_cast<const ToneMap&>(tone_map));
}
...
std::vector<std::complex<float>> OfdmModulator::build_ofdm_frame(
    ...
    return build_ofdm_frame(payload, len, tone_map);
}
...
std::vector<std::complex<float>> OfdmModulator::build_ofdm_frame(
    ...
    int n_codewords) {
    if (fec != tone_map.fec_rate ||
        n_codewords != tone_map.n_codewords) {
        IRIS_LOG("[OFDM-TX] frame shape mismatch: args fec=%d C=%d, map fec=%d C=%d",
                 static_cast<int>(fec), n_codewords,
                 static_cast<int>(tone_map.fec_rate), tone_map.n_codewords);
        return {};
    }
    return build_ofdm_frame(payload, len, tone_map);
}
