...
// Monotonic: O0-O13, no throughput inversions.
...
void OfdmSession::init(const NegotiatedPassband& passband, const IrisConfig& config) {
    ...
    ofdm_config_.clean_channel = config.ofdm_clean_channel;
    ofdm_config_.skip_papr_clip = config.ofdm_skip_papr_clip;
    ofdm_config_.llr_use_frame_nv = config.ofdm_llr_use_frame_nv;
    ...
    tx_tone_map_.use_nuc = nuc_enabled_;
...
std::vector<std::complex<float>> OfdmSession::build_tx_frame(const uint8_t* payload, size_t len) {
    ...
    ToneMap candidate = tx_tone_map_;
    candidate.fec_rate = fec;
    candidate.n_codewords = n_cw;

    auto iq = modulator_->build_ofdm_frame(payload, len, candidate);
    if (iq.empty())
        return {};

    tx_tone_map_ = std::move(candidate);
    n_frames_tx_++;
...
OfdmDemodResult OfdmSession::process_rx_frame(const std::complex<float>* iq, int n_samples) {
    ...
    if (result.success) {
        ...
        // Frame delivery and estimator authorization are distinct.  A payload
        // may succeed while a quality estimator is withheld; only a validated
        // selected CFO hypothesis may update session rate evidence.
        if (result.snr_db > 0 && result.cfo_resolved &&
...
void OfdmSession::update_speed_level(float snr_db) {
    ...
    if (new_level != speed_level_) {
        ...
        // Update tone map for new speed level
        LdpcRate new_fec = speed_level_to_fec(speed_level_);
        uint8_t preset = static_cast<uint8_t>(speed_level_ + 1);
        if (waterfill_enabled_ && tx_tone_map_.tone_map_id == 0) {
