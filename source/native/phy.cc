...
PhyConfig mode_a_config(float bandwidth_hz) {
    ...
    if (best_sps == 0)
        throw std::invalid_argument("bandwidth too narrow for Mode A");
...
PhyConfig mode_c_config() {
    ...
    return {19200, 5, Modulation::QAM16, RRC_ALPHA};
}
...
NativeModulator::NativeModulator(const PhyConfig& config, int sample_rate)
    ...
    config_.samples_per_symbol = sample_rate_ / config_.baud_rate;
