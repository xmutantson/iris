...
std::vector<float> Upconverter::iq_to_audio(const float* iq, size_t iq_count) {
    if (iq_count % 2 != 0)
        throw std::invalid_argument("IQ sample count must be even");
    ...
    for (size_t i = 0; i < n_samples; i++) {
        ...
        if (phase_ >= 2.0f * (float)M_PI || phase_ <= -2.0f * (float)M_PI)
            phase_ = std::fmod(phase_, 2.0f * (float)M_PI);
    }
...
Downconverter::Downconverter(float center_freq, int sample_rate)
    ...
    if (sample_rate_ == 0)
        throw std::invalid_argument("sample rate must be non-zero");
    if (center_freq_ == 0.0f)
        throw std::invalid_argument("center frequency must be non-zero");
    phase_inc_ = 2.0f * (float)M_PI * center_freq_ / (float)sample_rate_;
    ...
    float cutoff = center_freq * 0.95f;
    design_lpf(cutoff);
...
std::vector<float> Downconverter::audio_to_iq(const float* audio, size_t count) {
    if (count > std::vector<float>().max_size() / 2)
        throw std::length_error("audio sample count too large");
