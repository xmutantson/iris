...
void ChannelEqualizer::configure(const ProbeResult& my_rx_result,
    ...
    if (sample_rate <= 0) {
        IRIS_LOG("[CH-EQ] Invalid sample rate, skipping EQ");
        return;
    }
    ...
    float max_eq = *std::max_element(eq_curve_db.begin(), eq_curve_db.end());
    float min_eq = *std::min_element(eq_curve_db.begin(), eq_curve_db.end());
    ...
    int response_bins = 1;
    while (response_bins < N_TAPS * 4) response_bins <<= 1;
    float peak_gain = 0;
    for (int bin = 0; bin <= response_bins / 2; bin++) {
        float bin_gain = 0;
        for (int n = 0; n < N_TAPS; n++) {
            float phase = 2.0f * (float)M_PI * bin * (n - N_TAPS / 2) /
                          (float)response_bins;
            bin_gain += taps_[n] * std::cos(phase);
        }
        peak_gain = std::max(peak_gain, std::abs(bin_gain));
    }
    float max_gain = std::pow(10.0f, max_boost_db / 20.0f);
    if (peak_gain > max_gain) {
        float scale = max_gain / peak_gain;
...
void ChannelEqualizer::design_fir(const std::vector<float>& freq_hz,
    ...
    if (std::abs(actual_gain) > 1e-6f) {
        float scale = 1.0f / std::abs(actual_gain);
        for (auto& t : taps_) t *= scale;
    }
}
