...
std::vector<float> build_native_frame_internal(const uint8_t* payload, size_t len,
                                                ...
                                                LdpcRate fec, bool harq_flag) {
    if (len > NATIVE_MAX_PAYLOAD) return {};
...
std::vector<uint8_t> serialize_retx_descriptor(const HarqRetxDescriptor& desc) {
    ...
    size_t retx_bytes = total_bits / 8;
...
int detect_frame_start(const float* iq_samples, size_t count, int sps) {
    if (sps < 2) { g_last_best_corr = -1; return -1; }
...
bool decode_native_frame(const float* iq_samples, size_t count,
    ...
    g_last_fec = LdpcRate::NONE;
    g_last_mod = Modulation::BPSK;
    g_last_payload_len = 0;
    g_last_harq_flag = false;
    g_sym_phase_var.clear();
    ...
    if (fec != LdpcRate::NONE) {
        ...
        current_llrs = chase_attempted ? std::move(fresh_llrs) : std::move(llrs);
        ...
        scramble_bits(bits);
        if (bps >= 4 && (int)bits.size() >= bps) {
            int nbits = (int)bits.size();
            int ncols = nbits / bps;
            std::vector<uint8_t> debicm(nbits);
            for (int r = 0; r < bps; r++) {
                for (int c = 0; c < ncols; c++) {
                    debicm[r * ncols + c] = bits[c * bps + r];
                }
            }
            bits = std::move(debicm);
        }
    }
    ...
    if (rx_crc != calc_crc) {
        if (g_chase_active && fec != LdpcRate::NONE) {
            g_chase_llrs = std::move(current_llrs);
            IRIS_LOG("[DECODE] CRC failed — stored %zu LLRs for Chase combining",
                     g_chase_llrs.size());
        }
...
HarqDecodeResult decode_native_frame_harq(const float* iq_samples, size_t count,
    ...
    // Use the standard decode path without combining this burst with the copy
    // retained by the caller's failed decode.
    std::vector<uint8_t> payload;
    bool chase_active = g_chase_active;
    g_chase_active = false;
    bool ok = decode_native_frame(iq_samples, count, frame_start, default_config, payload);
    g_chase_active = chase_active;
