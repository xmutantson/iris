...
void hdlc_encode_raw(std::vector<uint8_t>& bits, const uint8_t* frame, size_t len,
    ...
    size_t reserve_size = bits.size();
    auto add_reserve = [&](size_t count, size_t scale) {
        if (count > (bits.max_size() - reserve_size) / scale) return false;
        reserve_size += count * scale;
        return true;
    };
    if (add_reserve(len, 10) &&
        add_reserve(preamble_flags > 0 ? static_cast<size_t>(preamble_flags) : 0, 8) &&
        add_reserve(postamble_flags > 0 ? static_cast<size_t>(postamble_flags) : 0, 8) &&
        add_reserve(32, 1))
...
bool HdlcDecoder::push_bit(int bit) {
    ...
    if (bit == 1) {
        ...
        if (ones_count_ == 6) {
            ...
            if (in_frame_ && bit_idx_ == 7 && buffer_.size() >= 2) {
    ...
    bit_idx_++;
    if (bit_idx_ == 8) {
        if (buffer_.size() >= AX25_MAX_FRAME + 2) {
            in_frame_ = false;
            buffer_.clear();
        } else {
