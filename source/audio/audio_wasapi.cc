...
public:
    ...
    bool start() override {
        if (sim_audio_guard_active()) {
            fprintf(stderr, "[SIM-AUDIO-GUARD] refusing to open real WASAPI "
                            "capture device while -x sim is active\n");
            return false;
        }
...
public:
    ...
    bool start() override {
        if (sim_audio_guard_active()) {
            fprintf(stderr, "[SIM-AUDIO-GUARD] refusing to open real WASAPI "
                            "playback device while -x sim is active\n");
            return false;
        }
        if (running_) return false;
        running_ = true;
...
// Backend selection stubs (Windows always uses WASAPI)
void set_audio_backend(AudioBackend) {}
AudioBackend get_audio_backend() { return AudioBackend::AUTO; }
