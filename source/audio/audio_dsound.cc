...
private:
    void capture_thread() {
        ...
        while (running_) {
            DWORD safe_read_pos = 0;
            buffer_->GetCurrentPosition(nullptr, &safe_read_pos);

            DWORD avail = (safe_read_pos >= read_pos) ?
                safe_read_pos - read_pos :
                buffer_bytes_ - read_pos + safe_read_pos;
    ...
    void playback_thread() {
        ...
        while (running_) {
            ...
            if (SUCCEEDED(hr)) {
                ...
                if (SUCCEEDED(buffer_->Unlock(p1, s1, p2, s2)))
