...
bool KissServer::start(int port, const std::string& bind_addr) {
    if (running_) return false;
    if (port < 0 || port > 65535) return false;

    // Resolve the bind address up front. A malformed value is fatal — we never
    // fall back to INADDR_ANY, which would silently expose the transmitter.
    uint32_t bind_host_order = 0;
    if (!parse_bind_address(bind_addr, bind_host_order)) {
        fprintf(stderr, "[KISS] Invalid bind address '%s' — refusing to start "
                        "(use a dotted IPv4 literal, or 0.0.0.0/any for all interfaces)\n",
                bind_addr.c_str());
        return false;
    }
    ...
    addr.sin_addr.s_addr = htonl(bind_host_order);
    ...
    printf("[KISS] Server listening on %u.%u.%u.%u:%d\n",
           (bind_host_order >> 24) & 0xFFu, (bind_host_order >> 16) & 0xFFu,
           (bind_host_order >> 8) & 0xFFu, bind_host_order & 0xFFu, port);
    return true;
}
