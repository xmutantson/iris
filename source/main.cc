...
extern int run_s1_repro();
extern int run_s2_gate();
extern int run_32qam_gate();
extern int run_wide6k_gate();
extern int run_tx_ofdm(int level, const char* outfile, bool dft_spread = true);
extern int run_rx_ofdm(int level, const char* infile, bool dft_spread = true);
extern int run_cpe_lens();
extern int run_mu_lens();
...
static void print_usage() {
    ...
    printf("  --tx-ofdm <level> <file>  Generate OFDM frames to raw S16LE file\n");
    printf("  --rx-ofdm <level> <file>  Decode OFDM frames from raw S16LE file\n");
    printf("  --no-dft-spread           Disable DFT-spread (SC-FDMA) for --tx/rx-ofdm\n");
    ...
    printf("  --no-preemph       Disable FM de-emphasis (flat audio / data port)\n");
    printf("  --ofdm-clean-channel  Use broadcast AWGN NV for MMSE/LLR on flat channels\n");
    printf("  --ofdm-skip-papr-clip Skip OFDM PAPR clipper (caution: Fe-Pi ADC interaction, see fact doc §7.3)\n");
...
#ifdef __linux__
    printf("  --alsa             Force ALSA audio backend (default on Linux)\n");
    printf("  --alsa-device <d>  ALSA device name (e.g., plughw:Audio)\n");
    printf("  --alsa-raw [hwdev] Bit-exact raw S32_LE snd_pcm backend (no plug/resample);\n");
    printf("                     per-dir env IRIS_ALSA_RAW_CAPTURE/_PLAYBACK (Loopback default)\n");
    printf("  --alsa-raw-smoke   Open + round-trip a buffer on the raw backend, then exit\n");
    printf("  --pulse            Force PulseAudio backend\n");
#endif
    printf("\nRadio/PTT:\n");
    ...
    printf("  --bind <addr>      Bind address for KISS/AGW servers\n");
    printf("                     (default: 127.0.0.1 = loopback only; 0.0.0.0 or 'any' = all interfaces)\n");
    printf("\nGUI:\n");
...
int main(int argc, char** argv) {
    ...
    bool cli_skip_tune = false;
    bool cli_force_ofdm = false;
    bool cli_no_preemph = false;
    bool cli_ofdm_clean_channel = false;
    bool cli_ofdm_skip_papr_clip = false;
    bool cli_ofdm_llr_use_frame_nv = false;
    ...
    int cli_tx_ofdm_level = -1;
    std::string cli_tx_ofdm_file;
    int cli_rx_ofdm_level = -1;
    std::string cli_rx_ofdm_file;
    bool cli_dft_spread = true;
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--test") == 0) {
            ...
            run_bench = true;
        } else if (strcmp(argv[i], "--s1repro") == 0) {
            run_s1 = true;
        } else if (strcmp(argv[i], "--s2gate") == 0) {
            run_s2 = true;
        } else if (strcmp(argv[i], "--gate32") == 0) {
            run_g32 = true;
        } else if (strcmp(argv[i], "--wide6k") == 0) {
            run_w6k = true;
            ...
            use_loopback = true;
                   ...
                   strcmp(argv[i + 1], "sim") == 0) {
            // "-x sim": software-channel backend that connects to Mercury's
            // sim_channel_relay.py (role/port from MERCURY_SIM_ROLE/_PORT env).
            i++;
            use_sim = true;
            ...
            cli_ax25_only = true;
        } else if (strcmp(argv[i], "--skip-tune") == 0) {
            cli_skip_tune = true;
        } else if (strcmp(argv[i], "--force-ofdm") == 0) {
            cli_force_ofdm = true;
        } else if (strcmp(argv[i], "--no-preemph") == 0) {
            cli_no_preemph = true;
        } else if (strcmp(argv[i], "--ofdm-clean-channel") == 0) {
            cli_ofdm_clean_channel = true;
        } else if (strcmp(argv[i], "--ofdm-skip-papr-clip") == 0) {
            cli_ofdm_skip_papr_clip = true;
        } else if (strcmp(argv[i], "--ofdm-llr-frame-nv") == 0) {
            cli_ofdm_llr_use_frame_nv = true;
            ...
            set_audio_backend(AudioBackend::ALSA);
        } else if (strcmp(argv[i], "--alsa-device") == 0 && i + 1 < argc) {
            set_audio_backend(AudioBackend::ALSA);
            set_alsa_device(argv[++i]);
        } else if (strcmp(argv[i], "--alsa-raw") == 0) {
            set_audio_backend(AudioBackend::ALSA_RAW);
            // Optional <hwdev> base device (per-direction env overrides it).
            if (i + 1 < argc && argv[i + 1][0] != '-')
                set_alsa_device(argv[++i]);
        } else if (strcmp(argv[i], "--alsa-raw-smoke") == 0) {
            set_audio_backend(AudioBackend::ALSA_RAW);
            cli_alsa_raw_smoke = true;
        } else if (strcmp(argv[i], "--pulse") == 0) {
            set_audio_backend(AudioBackend::PULSE);
#endif
        ...
        } else if (strcmp(argv[i], "--no-dft-spread") == 0) {
            cli_dft_spread = false;
        } else if (strcmp(argv[i], "--tx-ofdm") == 0 && i + 2 < argc) {
            cli_tx_ofdm_level = atoi(argv[++i]);
            cli_tx_ofdm_file = argv[++i];
        } else if (strcmp(argv[i], "--rx-ofdm") == 0 && i + 2 < argc) {
            cli_rx_ofdm_level = atoi(argv[++i]);
            cli_rx_ofdm_file = argv[++i];
        } else if (strcmp(argv[i], "--cpe-lens") == 0) {
            return run_cpe_lens();
        } else if (strcmp(argv[i], "--mu-lens") == 0) {
            return run_mu_lens();
    ...
    if (run_s2)
        return run_s2_gate();
    if (run_g32)
        return run_32qam_gate();
    if (run_w6k)
        return run_wide6k_gate();
    if (cli_tx_ofdm_level >= 0)
        return run_tx_ofdm(cli_tx_ofdm_level, cli_tx_ofdm_file.c_str(), cli_dft_spread);
    if (cli_rx_ofdm_level >= 0)
        return run_rx_ofdm(cli_rx_ofdm_level, cli_rx_ofdm_file.c_str(), cli_dft_spread);

#ifdef __linux__
    if (cli_alsa_raw_smoke)
        return alsa_raw_smoke_test();
#endif
    ...
    if (cli_skip_tune) config.skip_tune = true;
    if (cli_force_ofdm) { config.force_ofdm = true; config.skip_tune = true; }
    if (cli_no_preemph) config.ofdm_preemph_corner_hz = 0.0f;
    if (cli_ofdm_clean_channel) config.ofdm_clean_channel = true;
    if (cli_ofdm_skip_papr_clip) config.ofdm_skip_papr_clip = true;
    if (cli_ofdm_llr_use_frame_nv) config.ofdm_llr_use_frame_nv = true;
    ...
    config.bind_address = effective_bind_address(config.bind_address, cli_bind);
    ...
    printf("  KISS port: %d\n", config.kiss_port);
    printf("  TX level: %.2f\n", config.tx_level);
    // Digipeater role is an operator opt-in — print it loudly when active so
    // the modem never repeats third-party traffic silently ([Digipeat]).
    if (config.digipeat_enable) {
        std::string dcalls = config.digipeat_mycall.empty()
            ? (config.callsign +
               (config.ssid ? "-" + std::to_string(config.ssid) : std::string()))
            : config.digipeat_mycall;
        printf("  Digipeat: ENABLED for %s (connected=%s, ui=%s, alias=%s, wide=%s, dedupe=%ds)\n",
               dcalls.c_str(),
               config.digipeat_connected ? "on" : "off",
               config.digipeat_ui ? "on" : "off",
               config.digipeat_ui_alias.empty() ? "none" : config.digipeat_ui_alias.c_str(),
               config.digipeat_ui_wide.empty() ? "none" : config.digipeat_ui_wide.c_str(),
               config.digipeat_dedupe_ttl_s);
    }
    ...
    // Validate the KISS/AGW bind address once, up front. A malformed value is
    // fatal — we never silently widen the listeners to all interfaces, which
    // would hand any host that can reach this machine unauthenticated transmit
    // control of the radio under the operator's callsign.
    uint32_t bind_host_order = 0;
    if (!parse_bind_address(config.bind_address, bind_host_order)) {
        fprintf(stderr, "FATAL: invalid bind address '%s'. Use a dotted IPv4 "
                        "literal (e.g. 127.0.0.1) or 0.0.0.0 / any for all interfaces.\n",
                config.bind_address.c_str());
        return 1;
    }
    char bind_str[16];
    snprintf(bind_str, sizeof(bind_str), "%u.%u.%u.%u",
             (bind_host_order >> 24) & 0xFFu, (bind_host_order >> 16) & 0xFFu,
             (bind_host_order >> 8) & 0xFFu, bind_host_order & 0xFFu);
    bool bind_is_loopback = ((bind_host_order >> 24) == 127u);  // 127.0.0.0/8
    ...
    kiss.set_client_callback([&modem](int count) {
        ...
        modem.notify_local_client_count(count);
        if (count > 0) {
            for (const auto& result : modem.retained_transfer_results()) {
                if (!result) continue;
                IRIS_LOG("Custody recovery available: transfer=%llu outcome=%d "
                         "reason=%d unresolved_ranges=%zu unresolved_empty=%zu "
                         "partial_serialized=%zu",
                         static_cast<unsigned long long>(
                             result->transfer.transfer_id.value),
                         static_cast<int>(result->outcome),
                         static_cast<int>(result->reason),
                         result->preserved_unresolved_originals.size(),
                         result->preserved_unresolved_empty_records.size(),
                         result->preserved_incomplete_serialized_record.size());
            }
            if (const auto native_result = modem.last_arq_transfer_result()) {
                IRIS_LOG("Native custody recovery available: transfer=%llu "
                         "success=%d reason=%d preserved_records=%zu",
                         static_cast<unsigned long long>(native_result->transfer_id),
                         native_result->success ? 1 : 0,
                         static_cast<int>(native_result->reason),
                         native_result->preserved_records.size());
            }
        }
    });
    ...
    if (!kiss.start(config.kiss_port, config.bind_address)) {
        fprintf(stderr, "Failed to start KISS server on %s:%d\n", bind_str, config.kiss_port);
        return 1;
    }
    printf("  KISS server listening on %s:%d\n", bind_str, config.kiss_port);
    ...
    modem.set_rx_callback([&kiss, &agw, &modem, &config](const uint8_t* frame, size_t len) {
        ...
        const std::string& remote =
            modem.ax25_state() != Ax25SessionState::DISCONNECTED
                ? modem.ax25_remote_callsign() : modem.arq_remote_callsign();
        agw.send_to_clients(frame, len, remote, config.callsign);
    });

    // Retain a structured custody result independently of the AX.25 DISC/UA
    // notification.  The Modem keeps the immutable snapshot queryable; this
    // callback makes each newly published terminal result visible operationally.
    modem.set_transfer_result_callback([&modem](Modem::TransferResultPtr result) {
        if (!result) return;
        // Retrieve the retained instance instead of treating callback counts as
        // the report.  It remains available across reconnect until the client
        // explicitly disposes the exact TransferIdentity.
        const auto retained = modem.last_transfer_result();
        if (!retained ||
            retained->transfer.transfer_id.value !=
                result->transfer.transfer_id.value)
            IRIS_LOG("Custody transfer %llu retained behind an earlier failure",
                     static_cast<unsigned long long>(
                         result->transfer.transfer_id.value));
        IRIS_LOG("Custody transfer %llu terminal: outcome=%d reason=%d accepted=%zu "
                 "unresolved_ranges=%zu unresolved_empty=%zu partial_serialized=%zu",
                 static_cast<unsigned long long>(result->transfer.transfer_id.value),
                 static_cast<int>(result->outcome), static_cast<int>(result->reason),
                 result->original_records.size(),
                 result->preserved_unresolved_originals.size(),
                 result->preserved_unresolved_empty_records.size(),
                 result->preserved_incomplete_serialized_record.size());
        for (const auto& preserved : result->preserved_unresolved_originals) {
            IRIS_LOG("Custody unresolved: transfer=%llu record=%llu offset=%llu "
                     "length=%llu",
                     static_cast<unsigned long long>(
                         result->transfer.transfer_id.value),
                     static_cast<unsigned long long>(
                         preserved.range.record_id.value),
                     static_cast<unsigned long long>(preserved.range.offset),
                     static_cast<unsigned long long>(preserved.range.length));
        }
    });
    ...
    if (!agw.start(config.agw_port, config.bind_address)) {
        fprintf(stderr, "Warning: failed to start AGW server on %s:%d\n", bind_str, config.agw_port);
    } else {
        printf("  AGW server listening on %s:%d\n", bind_str, config.agw_port);
    }

    if (!bind_is_loopback) {
        printf("  *** WARNING: KISS/AGW are bound to %s (not loopback). Any host that\n", bind_str);
        printf("      can reach this address has UNAUTHENTICATED transmit control of the\n");
        printf("      radio under callsign %s. Bind to 127.0.0.1 unless this is intended.\n",
               config.callsign.c_str());
    }
    ...
    if (use_sim) {
        // Software channel: bridge passband audio to a second Iris stack over
        // Mercury's sim_channel_relay.py. No real audio device is opened.
        capture = create_sim_capture();
        playback = create_sim_playback();
    ...
    if (capture && playback) {
        ...
        if (cap_ok && play_ok) {
            bool cap_started = capture->start();
            bool play_started = playback->start();
            if (!cap_started || !play_started) {
                capture->stop();
                playback->stop();
                capture->close();
                playback->close();
                fprintf(stderr, "Audio: failed to start devices\n");
                return 1;
            }
            ...
            capture->close();
            playback->close();
            fprintf(stderr, "Audio: failed to open devices\n");
            return 1;
        }
    ...
    if (use_gui && !gui.init("Iris FM Data Modem")) {
        fprintf(stderr, "Failed to initialize GUI; continuing in console mode\n");
        use_gui = false;
    }
    ...
    modem.shutdown();
    rigctld_shutdown();
    bool config_saved = save_config(config_path, config);
    ...
    if (g_restart) {
        ...
        DWORD exe_path_len = GetModuleFileNameA(NULL, exe_path, MAX_PATH);
        if (exe_path_len == 0 || exe_path_len >= MAX_PATH) {
            fprintf(stderr, "Failed to find executable for restart (error %lu)\n",
                    GetLastError());
            return 1;
        }
        ...
        if (!CreateProcessA(exe_path, &cmd[0], NULL, NULL, FALSE,
                            0, NULL, NULL, &si, &pi)) {
            fprintf(stderr, "Failed to restart (error %lu)\n", GetLastError());
            return 1;
        }
