...
bool AgwServer::start(int port, const std::string& bind_addr) {
    ...
    if (!parse_bind_address(bind_addr, bind_host_order)) {
        fprintf(stderr, "[AGW] Invalid bind address '%s' — refusing to start "
    ...
    printf("[AGW] Server listening on %u.%u.%u.%u:%d\n",
